...
function Command({ className, ...props }: React.ComponentProps<typeof CommandPrimitive>) {
    return (
        <CommandPrimitive
            data-slot="command"
            className={cn(
                'size-full rounded-xl! bg-popover text-popover-foreground flex flex-col overflow-hidden p-1',
                className,
            )}
            {...props}
        />
...
function CommandItem({
    ...
    return (
        <CommandPrimitive.Item
            ...
            className={cn(
                "group/command-item outline-hidden in-data-[slot=dialog-content]:rounded-lg! data-selected:bg-muted data-selected:text-foreground [&_svg:not([class*='size-'])]:size-4 data-selected:**:[svg]:text-foreground relative flex cursor-default select-none items-center gap-2 rounded-sm px-2 py-1.5 text-sm data-[disabled=true]:pointer-events-none data-[disabled=true]:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0",
                className,
            )}
            {...props}
        >
